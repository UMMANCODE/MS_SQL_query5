...
(
    ...
    [Name] NVARCHAR(100) CHECK(
        LEN([Name]) BETWEEN 2 AND 100
    ),
    Price MONEY,
...
-- Additional tasks
-- 1. Scalar functions
CREATE FUNCTION FN_1 (@FirstName NVARCHAR(50), @LastName NVARCHAR(50)) RETURNS NVARCHAR(100) AS BEGIN
...
CREATE FUNCTION FN_2 (@AuthorId INT) RETURNS TABLE AS RETURN (
        SELECT *
FROM Authors AS A
WHERE A.Id = @AuthorId
    )
